...
class InferenceEngine:
    ...
    def __post_init__(self):
        ...
        try:
            with timer("Model loading"):
                self.__class__.model = foreign_class(**ModelConfigs.to_dict()).to(self.device)
    ...
    def _validate_input(self, 
                        audio_tensors: tuple[torch.Tensor, torch.Tensor]):
        ...
        if not (0 <= lengths).all() and (lengths <= 1).all():
            logging.error(f"Invalid length values detected: min={lengths.min()}, max={lengths.max()}")
            raise ValueError("Lengths must be between 0 and 1")

    def _put_on_device(self, data: tuple[torch.Tensor, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        try:
    ...
    def _flag_low_model_confidence(self,
                         score: torch.Tensor,
                         score_threshold: float = 0.9) -> torch.Tensor:
        try:
            ...
            if torch.any(low_conf_mask):
                ...
                logging.warning(f"Low confidence scores detected: {low_conf_scores.tolist()}")
    ...
    def _log_low_confidence_predictions(self,
                                        score: torch.Tensor,
                                        text_lab: str,
                                        confidence_flags: torch.Tensor):
          ...
          if low_conf_count > 0:
                ...
                for idx in low_conf_indices:
                 logging.warning(f"Sample {idx}: {text_lab[idx]} predicted with confidence {score[idx]:.4f}")
    ...
    def predict(
        ...
        try:
            ...
            waveform, lengths = audio_tensors
            logging.info(f"Memory usage before inference: {torch.cuda.memory_allocated()/1e9:.2f}GB")
            with timer("Model inference"):
                ...
                low_conf_count = confidence_flags.sum().item()
                if low_conf_count > 0:
                    self._log_low_confidence_predictions(score, text_lab, confidence_flags)
            logging.info(f"Memory usage after inference: {torch.cuda.memory_allocated()/1e9:.2f}GB")
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                logging.info(f"Memory usage after cleanup: {torch.cuda.memory_allocated()/1e9:.2f}GB")
